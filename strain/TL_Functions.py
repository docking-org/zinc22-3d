...
def get_matches_idx(tp, mol):
    ...
    for match in matches:
        ...
        if mol.GetAtomWithIdx(match[0]).GetSymbol()=='H' or mol.GetAtomWithIdx(match[3]).GetSymbol()=='H':
            continue
        idx.append([match[0], match[1], match[2], match[3]])
    hist_E = []
    hist_l = []
    hist_u = []
    angles = []
    if tp.get("method") == "exact":
        ...
        angles = [angle for angle in tp.find("angleList").findall("angle")]
    return idx, (hist_E, hist_l, hist_u), angles

class TL_Lookup:

    def __init__(self, mol, xml_tree):
        ...
        for HC in root.findall("hierarchyClass"):
            if HC.get("name") != "GG": #Not the general class
                for TP in HC.iter("torsionRule"): #Loop over each torsion rules
                    ...
                    idx_matches, histogram, angles = get_matches_idx(TP, mol)
                    if len(idx_matches) > 0:
                        #                      0            1          2       3           4  5                 6
                        self.match_idx.append((idx_matches, histogram, angles, "specific", i, TP.get("method"), TP.get("smarts")))
                    i += 1
        # Now for the general method:
        for TP in root.find("hierarchyClass[@name='GG']").iter("torsionRule"):
            idx_matches, histogram, angles = get_matches_idx(TP, mol)
            if len(idx_matches) > 0:
                self.match_idx.append((idx_matches, histogram, angles, "general", i, TP.get("method"), TP.get("smarts")))
            i += 1
    ...
    def lookup(self, mol):
        ...
        for match in self.match_idx:
            ...
            for idx_match in match[0]:
                
                r_1 = positions[idx_match[0]]
                r_2 = positions[idx_match[1]]
                r_3 = positions[idx_match[2]]
                r_4 = positions[idx_match[3]]

                theta = dihedral(r_1, r_2, r_3, r_4)

                if tor_method == "exact":

                    hist_E, hist_l, hist_u = histogram
                    ...
                    bond_info.append([
                    idx_match, #Convert tuple to list
                    theta,
                    smarts,
                    tor_class, #"general" or "specific"
                    "exact",
                    energy,
                    lower,
                    upper,
                    False, #This only could apply for the approximate method
                    tor_index #We will take this out when we create the final object
                    ])
