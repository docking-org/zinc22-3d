...
class Mol2(object):
  ...
  def processLine(self, line):
    ...
    if line[:17] == "@<TRIPOS>MOLECULE":
      ...
      if 1 == self.phase:
        ...
        #jklyu, 20200511, add strain terms
        #if len(line) > 1 and tokens[0][0:11] == "TotalStrain":
        #  self.inputEnergy.append(float(tokens[2]))
        #jklyu, 20200511, add strain terms
        elif len(line) > 1 and tokens[0][0:6] == "Strain":
          self.inputTotalStrain.append(float(tokens[2]))
          self.inputMaxStrain.append(float(tokens[3]))
          self.inputHydrogens.append(0)
          self.phase = 0 #rest of header ignored
      elif 2 == self.phase:
  ...
  def addStrainInfo(self, tE, pE):
    self.inputTotalStrain = []
    self.inputMaxStrain = []
    self.inputTotalStrain.extend(tE)
    self.inputMaxStrain.extend(pE)
    self.inputHydrogens.extend([0 for i in range(len(tE))])
  ...
  def copy(self):
    ...
    newM.inputTotalStrain = self.inputTotalStrain[:] #and this
    newM.inputMaxStrain = self.inputMaxStrain[:] #and this
  ...
  def keepConfsOnly(self, first, last):
    ...
    self.inputTotalStrain = self.inputTotalStrain[first:last]
    self.inputMaxStrain = self.inputMaxStrain[first:last]
  ...
  def writeMol2File(self, outFile, whichXyz=None):
    ...
    for oneXyz in whichXyz:
      ...
      outFile.write("SMALL\nUSER_CHARGES\n\n")
      if self.inputTotalStrain and oneXyz < len(self.inputTotalStrain):
        outFile.write("Strain = %.4f %.4f\n" % (self.inputTotalStrain[oneXyz], self.inputMaxStrain[oneXyz]))
      else:
      ...
      for oneBond in range(len(self.bondNum)):
        outFile.write( \
          ...
          (self.bondNum[oneBond], self.bondStart[oneBond], \
           self.bondEnd[oneBond], self.bondType[oneBond].ljust(2)))
