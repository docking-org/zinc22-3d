...
class Mol:
    def __init__(self,header,name,atom_list,bond_list,residue_list,bool_omega,str_omega):
        self.header       = str(header)
        self.name         = str(name)
        self.atom_list    = atom_list
        self.bond_list    = bond_list
        self.residue_list = residue_list
        self.bool_omega   = bool_omega
        self.str_omega    = str_omega # internal energy.
    ...
    for lnum in range(startline,len(lines)):
         ...
         if (len(linesplit) == 1):
            if(linesplit[0] == "@<TRIPOS>MOLECULE"):
               ...
               flag_substr = False
               flag_omega = False
               omega_internal_energy = ""

#USER_CHARGES
#
#mmff94s = 110.95

            if(linesplit[0] == "mmff94s"):
               flag_omega = True
               omega_internal_energy = line.strip()
    ...
    ID_heavy_atoms(atom_list)
    data = Mol('',Name,atom_list,bond_list,residue_list,flag_omega,omega_internal_energy)
    atom_list = [];bond_list = []
         ...
         if (len(linesplit) == 1):
            if(linesplit[0] == "@<TRIPOS>MOLECULE"):
               if flag_frist_mol:
                  ...
                  data = Mol(header2,Name,atom_list,bond_list,residue_list,flag_omega,omega_internal_energy)
                  mol_list.append(data)
                  atom_list = [];bond_list = []
               header2 = header1
               header1 = ''
